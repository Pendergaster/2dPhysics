...
static void inline orthomat_2d(mat4* ortho, float left, float right, float bottom, float top)
{
// NOTE IT IS [X][Y]
identity(ortho);
ortho->mat[0][0] = (2.f / (right - left));
...
static vec2 inline rotate_point2D(float angle, vec2 point,const vec2 pos)
{
	point.x -= pos.x;
	point.y -= pos.y;
